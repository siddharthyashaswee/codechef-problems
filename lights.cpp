...
{
  register short i;
  ifp = fopen("LIGHTS.INP","r");
  fscanf(ifp,"%hd %hd\n",&from,&to);
  fscanf(ifp,"%hd %hd\n",&V,&E);
  dad = (short *)malloc ((V+1) * sizeof (short));
  val = (long *)malloc ((V+1) * sizeof (long));
  for (i = 1; i <= V; i++) {
    dad[i] = 0;
    val[i] = 0;
  }
  light = (struct lighttype *)malloc ((V+1) * sizeof (struct lighttype));
  get_vertices();
...
{
  short i = 0, j, x;
  resar = (short *)malloc((V+1) * sizeof(short));
